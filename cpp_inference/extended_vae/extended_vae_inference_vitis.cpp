...
typedef ap_fixed<16, 2, AP_RND, AP_SAT> angle_t;
typedef ap_fixed<18, 2, AP_RND, AP_SAT> amp_t;
typedef ap_fixed<24, 4, AP_RND, AP_SAT> acc_t;
typedef ap_fixed<18, 2, AP_RND, AP_SAT> output_t;
typedef ap_fixed<16, 4, AP_RND, AP_SAT> input_t;  // Pre-normalized angles
...
#include "vitis_extended_vae_weights.h"
#include "vitis_extended_vae_trig_luts.h"

// ========================== LUT indexing ==========================
// Extended VAE uses physics-aware normalization (angles in radians)

// For eta/phi: [-π, π] -> LUT index
inline ap_uint<TRIG_LUT_BITS> lut_index_eta_phi(input_t theta) {
    #pragma HLS INLINE
    ...
    return (ap_uint<TRIG_LUT_BITS+4>)(u * (TRIG_LUT_SIZE - 1));
}

// For pt: [0, π] -> LUT index
inline ap_uint<TRIG_LUT_BITS> lut_index_pt(input_t theta) {
    #pragma HLS INLINE
...
inline void get_cs_eta_phi(input_t theta, angle_t& c, angle_t& s) {
    #pragma HLS INLINE
    ap_uint<TRIG_LUT_BITS> idx = lut_index_eta_phi(theta);
...
inline void get_cs_pt(input_t theta, angle_t& c, angle_t& s) {
    #pragma HLS INLINE
    ap_uint<TRIG_LUT_BITS> idx = lut_index_pt(theta);
    c = pt_lut_cos[idx];
    s = pt_lut_sin[idx];
}
...
// Include vitis_extended_vae_classical.h for dense layer weights

#include "vitis_extended_vae_classical.h"

typedef ap_fixed<20, 4, AP_RND, AP_SAT> score_t;
...
void dense_hidden(output_t quantum_out[32], output_t hidden[HIDDEN_DIM]) {
    #pragma HLS INLINE off
    #pragma HLS ARRAY_PARTITION variable=quantum_out complete
...
void compute_z_mean(output_t hidden[HIDDEN_DIM], output_t z_mean[LATENT_DIM]) {
    #pragma HLS INLINE off
    #pragma HLS ARRAY_PARTITION variable=hidden complete
    #pragma HLS ARRAY_PARTITION variable=z_mean complete
...
void extended_vae_quantum_encoder_top(
    input_t features[56],
    output_t encoder_output[32]
) {
    #pragma HLS INTERFACE mode=s_axilite port=return
    #pragma HLS INTERFACE mode=s_axilite port=features
    #pragma HLS INTERFACE mode=s_axilite port=encoder_output
    
    #pragma HLS ARRAY_PARTITION variable=features complete
    #pragma HLS ARRAY_PARTITION variable=encoder_output complete
    ...
    #pragma HLS ARRAY_PARTITION variable=met_out complete
    #pragma HLS ARRAY_PARTITION variable=ele_out complete
    #pragma HLS ARRAY_PARTITION variable=mu_out complete
    #pragma HLS ARRAY_PARTITION variable=jet_out complete
...
void extended_vae_anomaly_top(
    ...
    #pragma HLS ARRAY_PARTITION variable=quantum_out complete
    extended_vae_quantum_encoder_top(features, quantum_out);
