#!/usr/bin/env python3
"""
Export fixed-point headers for Extended VAE Vitis HLS.
Generates:
  - vitis_extended_vae_trig_luts.h: Fixed-point trig LUTs for physics-aware normalization
  - vitis_extended_vae_weights.h: Fixed-point trainable weights with depth constants
"""
...
def generate_trig_luts(out_path, nbits=10):
    ...
    with open(out_path, "w") as f:
        f.write("// Auto-generated fixed-point trig LUTs for Extended VAE Vitis HLS\n")
        ...
        emit_ap_fixed_array(f, "angle_t", "eta_phi_lut_cos", eta_phi_c)
        emit_ap_fixed_array(f, "angle_t", "eta_phi_lut_sin", eta_phi_s)
        emit_ap_fixed_array(f, "angle_t", "pt_lut_cos", pt_c)
        emit_ap_fixed_array(f, "angle_t", "pt_lut_sin", pt_s)
...
def generate_weights(ckpt_path, out_path):
    ...
    # MET: 3 weights per depth (RX, RY, RZ)
    # Ele/Mu: 8 weights per depth (4 RX + 4 RY)
    # Jet: 13 weights per depth (10 RX + 3 RY)
    ...
    with open(out_path, "w") as f:
        f.write("// Auto-generated fixed-point weights for Extended VAE Vitis HLS\n")
        f.write("#pragma once\n\n")
        f.write('#include <ap_fixed.h>\n\n')
        f.write("typedef ap_fixed<16, 2, AP_RND, AP_SAT> angle_t;\n\n")
...
def main():
    ...
    parser.add_argument("--out-dir", default="vitis_extended_vae_headers", help="Output directory")
